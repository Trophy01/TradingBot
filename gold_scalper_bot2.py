...
class GoldScalperBot:
   ...
   # --- Configuration ---
    ...
    SL_POINTS = 500 # Increased from 300 to allow more room for a trade to move
    TP_POINTS = 500 # Decreased from 1000 to hit the target faster
    ...
    STOCHASTIC_OVERBOUGHT = 75 # Lowered from 80
    STOCHASTIC_OVERSOLD = 25 # Raised from 20

    # Dynamic Aggressive Loss Limiter Configuration
    AGGRESSIVE_SL_PERCENT_TRIGGER = 0.20 # Increased from 10%
    AGGRESSIVE_SL_FIXED_BUFFER_FOR_TRIGGER = 15 # Increased from 10
    AGGRESSIVE_SL_MIN_DISTANCE_FROM_CURRENT = 15 # Increased from 10

    # S/R Lookback for Aggressive SL
    SR_LOOKBACK_BARS = 30 # Decreased from 50
    SR_BREACH_BUFFER_POINTS = 10 # Increased from 5

    # Absolute Maximum Adverse Excursion to Close Trade
    MAX_ADVERSE_EXCURSION_CLOSE_POINTS = 200 # Increased from 150

    # ATR Trailing Stop Configuration
    ATR_PERIOD = 14 # Changed from 20
    ATR_MULTIPLIER = 2.0 # Increased from 1.5 for a wider trailing stop

    # Cooldown Configuration
    COOLDOWN_AFTER_TRADE_SECONDS = 60 # Drastically reduced from 300
    COOLDOWN_LOSS_THRESHOLD_USD = 0.00 # Changed from 0.01
    COOLDOWN_POINTS_THRESHOLD = 0 # Changed from 5

    # --- Trade Filters ---
    MAX_SPREAD_POINTS = 50 # Increased from 25
    MAX_HOLD_TIME_SECONDS = 300 # Increased from 180 to give trades more time
    NUM_CONCURRENT_TRADES = 5 # Increased from 3
    TRADE_ENTRY_COOLDOWN_SECONDS = 5 # Drastically reduced from 30

    # --- Dynamic Take Profit Configuration ---
    DYNAMIC_TP_ATR_MULTIPLIER = 2.0 # Decreased from 3.0
    MIN_DYNAMIC_TP_POINTS = 50 # Decreased from 100
    MAX_DYNAMIC_TP_POINTS = 1500 # Decreased from 2000

    # --- Partial Profit Taking Configuration ---
    PARTIAL_PROFIT_TRIGGER_POINTS = 150 # Decreased from 250
    PARTIAL_PROFIT_PERCENTAGE = 0.25 # Decreased from 0.50

    # --- Trend Filter Configuration ---
    TREND_MA_PERIOD = 100 # Decreased from 200
    TREND_MA_TIMEFRAME = mt5.TIMEFRAME_M5 # Kept the same
    MA_SLOPE_LOOKBACK_BARS = 2 # Decreased from 3

    # --- Reversal Confirmation Configuration (NEW) ---
    REVERSAL_CONFIRMATION_BARS = 1 # Decreased from 2
